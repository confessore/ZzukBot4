...
namespace ZzukBot.Launcher
{
    internal class Program
    {
        ...
        private static bool IsLocked(string parPathAndFile)
        {
            ...
            {
                Console.WriteLine("File \"" + parPathAndFile + "\" is still in use.");
                return true;
            }
        }
        ...
        private static void Update()
        {
            ...
            try
            {
                ...
                using (var webClient = new WebClient())
                {
                    ...
                    webClient.DownloadFile("https://www.zzukbot.org/downloads/zzukbot.zip", zipTempFilePath);
                }
            }
            catch(Exception e)
            {
            ...
            foreach (var newPath in Directory.GetFiles(updateFolder, "*.*",
                SearchOption.AllDirectories))
            {
                if (newPath.Contains("Launcher.exe")) continue;
